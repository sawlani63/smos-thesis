...
use smos_sddf::serial_config::SDDF_NAME_LENGTH;
use smos_sddf::serial_config::SDDF_SERIAL_BEGIN_STR_MAX_LEN;
...
use smos_sddf::{
    ...
    serial_config::{
        SerialConnectionResource, SerialVirtTxClientConfig, SerialVirtTxConfig, SDDF_SERIAL_MAGIC,
    },
};
extern crate alloc;
use alloc::vec::Vec;
...
extern "C" {
    static mut config: SerialVirtTxConfig;
}
...
const client_name_cstr: &CStr = c"client0";
const begin_str_cstr: &CStr = c"Begin input\n";

#[smos_declare_main]
fn main(rs_conn: RootServerConnection, mut cspace: SMOSUserCSpace) {
    sel4::debug_println!("Hello, I am serial_tx_virt!!!");
    ...
    unsafe {
        let mut clients: [MaybeUninit<SerialVirtTxClientConfig>; SDDF_SERIAL_MAX_CLIENTS] =
            MaybeUninit::uninit().assume_init();

        let mut name: [c_char; SDDF_NAME_LENGTH] = [0; SDDF_NAME_LENGTH];
        for (i, c) in client_name_cstr.to_bytes().into_iter().enumerate() {
            name[i] = *c as i8;
        }

        let mut begin_str: [c_char; SDDF_SERIAL_BEGIN_STR_MAX_LEN] =
            [0; SDDF_SERIAL_BEGIN_STR_MAX_LEN];
        for (i, c) in begin_str_cstr.to_bytes().into_iter().enumerate() {
            begin_str[i] = *c as i8;
        }

        clients[0] = MaybeUninit::new(SerialVirtTxClientConfig {
            name: name,
            conn: SerialConnectionResource {
        ...
        config = SerialVirtTxConfig {
            ...
            num_clients: 1,
            begin_str: begin_str,
            begin_str_len: (begin_str_cstr.to_bytes().len()).try_into().unwrap(),
            enable_colour: true,
            enable_rx: true,
        }
    }
