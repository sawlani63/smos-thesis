...
impl ServerReceivedHandleOrHandleCap {
	pub fn new_handle(idx: usize) -> Self {
		return ServerReceivedHandleOrHandleCap::Handle(
			ReceivedHandle {
				idx: idx
		});
	}
	...
	pub fn new_unwrapped_handle_cap(idx: usize) -> Self {
		return ServerReceivedHandleOrHandleCap::UnwrappedHandleCap(
			UnwrappedHandleCap {
				idx: idx
		});
	}
	...
	pub fn new_wrapped_handle_cap(cptr: sel4::AbsoluteCPtr) -> Self {
		return ServerReceivedHandleOrHandleCap::WrappedHandleCap(
			WrappedHandleCap {
				cptr: cptr
		});
	}
...
impl<A: HandleType> TryFrom<ServerReceivedHandleOrHandleCap> for HandleCap<A> {
	...
	fn try_from(val: ServerReceivedHandleOrHandleCap) -> Result<Self, Self::Error> {
		match val {
			ServerReceivedHandleOrHandleCap::WrappedHandleCap(wrapped_hndl_cap) => Ok(HandleCap::new(wrapped_hndl_cap.cptr)),
			_ => Err(())
		}
	}
}
