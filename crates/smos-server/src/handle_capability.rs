use crate::handle::{ServerHandle, HandleInner};
...
impl<T: HandleInner> HandleCapabilityTable<T> {
	...
	pub fn allocate_handle_cap(&mut self) -> Result<(usize, &mut Option<ServerHandle<T>>, Option<sel4::AbsoluteCPtr>), InvocationError> {
