...
pub fn handle_error(ipc_buf: &mut sel4::IpcBuffer, error: InvocationError) -> sel4::MessageInfo {
    ...
    msginfo = match error {
        ...
        InvocationError::InvalidInvocation => msginfo.label(InvocationErrorLabel::InvalidInvocation.into()),
        ...
        InvocationError::OutOfHandleCaps => msginfo.label(InvocationErrorLabel::OutOfHandleCaps.into()),
        InvocationError::AlignmentError{which_arg} => {
            ipc_buf.msg_regs_mut()[AlignmentErrorMessage::Which as usize] = which_arg.try_into().unwrap();
            msginfo.label(InvocationErrorLabel::AlignmentError as u64).length(AlignmentErrorMessage::Length.into())
        },
        InvocationError::InvalidArguments => msginfo.label(InvocationErrorLabel::InvalidArguments.into()),
        InvocationError::InvalidHandle{which_arg} => {
            ipc_buf.msg_regs_mut()[InvalidHandleMessage::Which as usize] = which_arg.try_into().unwrap();
            msginfo.label(InvocationErrorLabel::InvalidHandle as u64).length(InvalidHandleMessage::Length.into())

        },
        InvocationError::InvalidHandleCapability{which_arg} => {
            ipc_buf.msg_regs_mut()[InvalidHandleCapabilityMessage::Which as usize] = which_arg.try_into().unwrap();
            msginfo.label(InvocationErrorLabel::InvalidHandleCapability as u64).length(InvalidHandleCapabilityMessage::Length.into())
        },
        InvocationError::DataBufferNotSet => msginfo.label(InvocationErrorLabel::DataBufferNotSet.into()),
        InvocationError::BufferTooLarge => msginfo.label(InvocationErrorLabel::BufferTooLarge.into()), // @alwin: This might eventually need a way to specify which arg
        InvocationError::InsufficientResources => msginfo.label(InvocationErrorLabel::InsufficientResources.into()),
