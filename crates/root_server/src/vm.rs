...
use smos_server::ntfn_buffer::{VMFaultNotification, NotificationType, NtfnBufferData, enqueue_ntfn_buffer_msg};
...
fn forward_vm_fault(p: &mut UserProcess, view: Rc<RefCell<View>>,
					offset: usize, reply: ReplyWrapper, info: sel4::VmFault) {
...
pub fn handle_page_map(cspace: &mut CSpace, ut_table: &mut UTTable, frame_table: &mut FrameTable,
					   p: &mut UserProcess, args: &PageMap) -> Result<SMOSReply, InvocationError> {
	...
	let window_reg_ref = p.get_handle(args.window_registration_hndl.idx).or(Err(InvocationError::InvalidHandle {which_arg: 0}))?;
	...
	let src_win = p.find_window_containing(args.content_vaddr).ok_or(InvocationError::InvalidArguments)?;
	let src_window_offset = (args.content_vaddr - src_win.borrow_mut().start);

	/* Check that the window has a view associated with it */
	let src_view = src_win.borrow_mut().bound_view.as_ref().ok_or(InvocationError::InvalidArguments)?.clone();
	...
	if src_view.borrow().caps[src_window_offset / PAGE_SIZE_4K].is_none() {
		...
		let view_frame_slot = cspace.alloc_slot().expect("@alwin: this should not be an assert");
        let view_frame_cap = sel4::CPtr::from_bits(view_frame_slot.try_into().unwrap()).cast::<sel4::cap_type::UnspecifiedFrame>();
	...
	if dst_view.borrow().pending_fault.is_some() {
		...
		map_frame(cspace, ut_table, dst_view.borrow().caps[args.view_offset / PAGE_SIZE_4K].unwrap(),
				  dst_view.borrow().pending_fault.as_ref().unwrap().2,
			  	  ...
			  	  dst_view.borrow().rights.clone(), sel4::VmAttributes::DEFAULT, None
		);
...
pub fn handle_vm_fault(cspace: &mut CSpace, frame_table: &mut FrameTable, ut_table: &mut UTTable,
				   	   reply: ReplyWrapper, proc: &mut UserProcess,
				   	   fault_info: sel4::VmFault) -> Option<FaultReply> {
	...
	if window.is_none() {
		warn_rs!("Process faulted at vaddr: 0x{:x}, which is not inside a window", fault_info.addr());
	...
	if window_unwrapped.borrow().bound_view.is_none() {
		warn_rs!("Process faulted at vaddr: 0x{:x}, which does not have a bound view", fault_info.addr());
		return Some(FaultReply::VMFault{resume: false});
	}
	...
	if view.borrow().caps[fault_offset / PAGE_SIZE_4K].is_none() {
		...
		if object_option.as_ref().is_none() {
			forward_vm_fault(proc, view.clone(),
