...
impl View {
	pub fn new(bw: Rc<RefCell<Window>>, bo: Option<Rc<RefCell<AnonymousMemoryObject>>>,
			   msi: Option<(Rc<RefCell<Server>>, usize, usize)>, rights: sel4::CapRights,
			   win_off: usize, obj_off: usize) -> View {
	...
	pub fn insert_cap_at(&mut self, offset: usize, cap: sel4::cap::UnspecifiedFrame) -> Result<(), sel4::Error> {
		let mut shift = 39;
		let mut curr_table_lvl = 0;
		...
		while curr_table_lvl < 3 {
			...
			if curr_table[idx].is_none() {
				curr_table[idx] = Some(ViewCapTableEntry::CapTable(ViewCapTable { table: vec![None; OBJ_LVL_MAX] }));
			}
		...
		match &curr_table[idx] {
			...
			Some(x) => {
				match x {
					ViewCapTableEntry::Cap(_) => return Err(sel4::Error::DeleteFirst),
					ViewCapTableEntry::CapTable(_) => panic!("Internal error: FrameTable on bottom level should not occur")
				}
			}
		}
	}
	...
	fn cleanup_cap_table_inner(vec: &Vec<Option<ViewCapTableEntry>>, cspace: &mut CSpace, delete: bool) {
		for node in vec {
			match node {
				None => continue,
				Some(x) => {
					match x {
						ViewCapTableEntry::CapTable(ref y) => Self::cleanup_cap_table_inner(&y.table, cspace, delete),
