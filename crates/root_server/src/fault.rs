...
pub fn handle_fault(cspace: &mut CSpace, frame_table: &mut FrameTable, ut_table: &mut UTTable,
                    reply: ReplyWrapper, msg: sel4::MessageInfo, pid: usize, ) -> Option<sel4::MessageInfo> {

    let mut p = procs_get_mut(pid).as_mut().expect("Was invoked with invalid badge").borrow_mut();
    ...
    let ret = match fault {
        ...
        sel4::Fault::VmFault(f) => {
            handle_vm_fault(cspace, frame_table, ut_table, reply, &mut p, f)
        },
    }?;
