...
pub fn pop(head: Option<*mut UT>) -> Result<(Option<*mut UT>, *mut UT), sel4::Error> {
    ...
    unsafe { return Ok(((*popped).next, popped)) }
}
...
impl UTTable {
    pub fn alloc_4k_untyped(self: &mut Self) -> Result<(usize, UTWrapper), sel4::Error> {
        ...
        let list =
            self.free_untypeds[SIZE_BITS_TO_INDEX(sel4_sys::seL4_PageBits.try_into().unwrap())];
    ...
    pub fn map_frame_to_next_free_vaddr(
        self: &mut Self,
        cspace: &mut CSpace,
        frame: sel4::cap::SmallPage,
    ...
    pub fn ensure_new_structures(self: &mut Self, cspace: &mut CSpace) -> Result<(), sel4::Error> {
        ...
        if self.free_structures == None || unsafe { (*self.free_structures.unwrap()).next == None }
        {
            ...
            let frame =
                CPtr::from_bits(cptr.try_into().unwrap()).cast::<sel4::cap_type::SmallPage>();
    ...
    pub fn free(self: &mut Self, ut: UTWrapper) {
        self.free_untypeds[SIZE_BITS_TO_INDEX(ut.get_size_bits())] = push_wrapper(
    ...
    pub fn alloc(
        ...
        if size_bits < sel4_sys::seL4_EndpointBits.try_into().unwrap() {
            err_rs!(
                "UT Table cannot alloc untyped < {:x} in size",
                sel4_sys::seL4_EndpointBits
            );
            return Err(sel4::Error::InvalidArgument);
        }
        ...
        if head.is_none() {
            ...
            unsafe {
                (*new1).cap =
                    CPtr::from_bits(cslot1.try_into().unwrap()).cast::<sel4::cap_type::Untyped>();
            ...
            unsafe {
                (*new2).cap =
                    CPtr::from_bits(cslot2.try_into().unwrap()).cast::<sel4::cap_type::Untyped>();
                (*new2).size_bits = size_bits.try_into().unwrap();
            }

            // Untype the larger object into the two new smaller objects
            if let Err(e) = cspace.untyped_retype(
                &larger.get_cap(),
            ...
            self.free_untypeds[SIZE_BITS_TO_INDEX(size_bits)] =
                push(self.free_untypeds[SIZE_BITS_TO_INDEX(size_bits)], new1);
            self.free_untypeds[SIZE_BITS_TO_INDEX(size_bits)] =
                push(self.free_untypeds[SIZE_BITS_TO_INDEX(size_bits)], new2);
        }
    ...
    pub fn new(memory: usize, region: UTRegion, next_free_vaddr: usize) -> UTTable {
        UTTable {
            first_paddr: region.start,
            untypeds: Some(memory as *mut UT),
            free_untypeds: [None; N_UNTYPED_LISTS],
            n_4k_untyped: 0,
            free_structures: None,
    ...
    fn paddr_to_ut(self: &Self, paddr: usize) -> *mut UT {
        return self
            .untypeds
            .unwrap()
            .wrapping_add((paddr - self.first_paddr) / PAGE_SIZE_4K);
    }

    fn ut_to_paddr(self: &Self, ut: *const UT) -> usize {
        (unsafe {
            <isize as TryInto<usize>>::try_into(ut.offset_from(self.untypeds.unwrap())).unwrap()
        }) * PAGE_SIZE_4K
            + self.first_paddr
    }

    pub fn add_untyped_range(
        self: &mut Self,
        paddr: usize,
        mut cptr: usize,
        n_pages: usize,
        ...
        for i in 0..n_pages {
            ...
            unsafe {
                (*node).cap =
                    CPtr::from_bits(cptr.try_into().unwrap()).cast::<sel4::cap_type::Untyped>();
                (*node).valid = true;
            }
            cptr += 1;
            if !device {
                ...
                self.free_untypeds[SIZE_BITS_TO_INDEX(sel4_sys::seL4_PageBits as usize)] =
                    push(list, node);
                self.n_4k_untyped += 1;
            }
        }
