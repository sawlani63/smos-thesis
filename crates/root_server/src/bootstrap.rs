...
fn calculate_ut_caps(bi : &sel4::BootInfo, size_bits: u32, bootinfo_avail_bytes: &[usize]) -> usize {
...
fn steal_untyped(bi: &sel4::BootInfo, size_bits: usize, bootinfo_avail_bytes: &[usize]) -> Option<(sel4::LocalCPtr<sel4::cap_type::Untyped>, usize)> {
	...
	for (i, untyped) in bi.untyped_list().iter().enumerate() {
		if untyped_in_range(untyped) && bootinfo_avail_bytes[i] >= BIT(size_bits) {
			paddr = paddr_from_avail_bytes(bi, i, size_bits, bootinfo_avail_bytes);
		}
...
pub fn smos_bootstrap(bi: &sel4::BootInfo) -> CSpace{
	...
	let bootinfo_avail_bytes : [usize; sel4_config::consts::MAX_NUM_BOOTINFO_UNTYPED_CAPS] = {0};
	...
	let n_slots: usize = bi.empty().start - 1;
	...
	let size : usize = ut_pages * PAGE_SIZE_4K;
    ...
    n_slots += calculate_ut_caps(bi, sel4_sys::seL4_PageBits, &bootinfo_avail_bytes);
    ...
    let (ut_cptr, _) = steal_untyped(bi, BYTES_TO_SIZE_BITS(size) + 1, &bootinfo_avail_bytes).expect("Not enough memory");
    ...
    let mut total: usize = n_cnodes;
        let blueprint = sel4::ObjectBlueprint::CNode{
    	size_bits: CNODE_SLOT_BITS(CNODE_SIZE_BITS),
    };
    let lvl1_cptr_cnode = sel4::BootInfo::init_cspace_local_cptr::
    					  <sel4::cap_type::CNode>(lvl1_cptr);

    /* now create the 2nd level nodes, directly in the node we just created */
    while total > 0 {
    	chunk = usize::min(sel4_config::consts::RETYPE_FAN_OUT_LIMIT, total);
    ...
    boot_cptr_cap.copy(&sel4::BootInfo::init_thread_cnode().relative_self(), sel4::CapRightsBuilder::all().build());
    ...
    let init_cnode_cap = lvl1_cptr_cnode.relative_bits_with_depth(sel4_sys::seL4_RootCNodeCapSlots::seL4_CapInitThreadCNode, depth);
